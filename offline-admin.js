...
function loadDetailedBatchesFromStore() {
    ...
    const batches = window.BatchStore.getAll('OFFLINE');
...
async function saveDetailedBatch() {
    ...
    if (editingId) {
        ...
        const existing = window.BatchStore.getAll('OFFLINE').find(b => b.id === editingId);
        batchData = { ...existing, ...batchData };
    } else {
        batchData.id = 'batch-off-' + Date.now();
        batchData.modules = [];
        batchData.mode = 'Offline';
    }

    try {
        const success = window.BatchStore.save('OFFLINE', batchData);
...
async function deleteDetailedBatch(batchId) {
    ...
    try {
        const success = window.BatchStore.delete('OFFLINE', batchId);
